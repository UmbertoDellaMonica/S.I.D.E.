...
\end{itemize}
  ...
  \begin{figure}[!ht]
    \centering
    \includegraphics[width=0.8\textwidth]{img/side-env.png}
    \caption{Struttura concettuale dell'Architettura SCADA}
    \label{fig:side_architecture}
  \end{figure}
...
\subsection{Panoramica dell' IDS}
...
\begin{lstlisting}[language=Python, caption={Funzione Parsing Modbus Payload}, label={lst:parisng_modbus_function}]
...
\begin{lstlisting}[language=Python, caption={Funzione Parsing S7 Payload}, label={lst:parisng_s7_function}]
...
\begin{lstlisting}[language=Python, caption={Funzione Parsing OPC-UA Payload}, label={lst:parisng_opcua_function}]
...
\begin{enumerate}
  ...
  \item Al superamento della soglia di aggregazione viene generato un evento di allarme di warning.
\end{enumerate}



\subsection{Modulo di Anomaly Detection: ML - Isolation Forest}
\label{subsec:ml-isoforest}

Il modulo \texttt{IsolationForestService} rappresenta il componente di rilevazione anomalie basato su apprendimento automatico del sistema \texttt{S.I.D.E.}, complementare al servizio statistico \texttt{ZScoreService}.
La sua funzione è quella di individuare pattern di traffico anomali attraverso un modello di \textit{Machine Learning} addestrato su flussi di pacchetti SCADA etichettati come “attacco” e “clean”, sfruttando la capacità dell’algoritmo \texttt{Isolation Forest} di isolare istanze rare o inconsuete nello spazio delle feature.

\subsubsection{Architettura del Modulo e Dataset di Addestramento}
Il processo di training del modello \texttt{IsolationForest} avviene in una fase offline, utilizzando un insieme di dataset CSV derivati da catture di traffico SCADA in due condizioni:
\begin{itemize}
\item \textbf{Dataset Clean:} rappresenta il comportamento legittimo dei dispositivi industriali;
\item \textbf{Dataset Attack:} contiene diverse tipologie di attacchi (es. \textit{Replay}, \textit{Function Code Injection}, \textit{Flooding}).
\end{itemize}

Durante il training, il sistema concatena i file CSV d’attacco, ne utilizza il 70% per l’addestramento e il restante per la validazione.
Le feature di interesse, selezionate in funzione del protocollo (es. Modbus/TCP), comprendono:


\begin{lstlisting}[language=python, caption={Selezione delle Feature per ML-IsolationForestModbusService}]
features = ["pkt_size","ip_ttl","ip_len","ip_proto_num","src_port","dst_port","modbus_function_code","modbus_data_length"]
\end{lstlisting}


Tali feature vengono pulite, normalizzate con uno \texttt{StandardScaler} e successivamente utilizzate per addestrare il modello di isolamento.
Al termine, vengono generati e salvati su disco i seguenti artefatti:
\begin{itemize}
    \item \texttt{isoforest\_modbus\_model.joblib}: modello addestrato;
    \item \texttt{isoforest\_modbus\_scaler.joblib}: scaler associato per la normalizzazione dei dati;
    \item \texttt{isoforest\_modbus\_threshold.txt}: soglia di decisione derivata dal training.
\end{itemize}


\subsubsection{Meccanismo di Rilevazione}
Durante l’analisi online, ciascun \textit{Protocol Handler} (es. ModbusHandler) richiama il servizio \texttt{IsolationForestModbusService} tramite il metodo:


\begin{lstlisting}[language=python, caption={Utilizzo e integrazione del Modello di Machine Learning all'interno dell'Handler}]
IsolationForestModbusService.get_instance().check(parsed_packet, raw_packet)
\end{lstlisting}


Il modulo opera come una \textit{singleton class}, che carica all’avvio il modello, lo scaler e la soglia associati al protocollo in uso.
Per ogni pacchetto, viene invocato un \textit{feature extractor} che ricostruisce il vettore di input del modello a partire da:
\begin{itemize}
\item campi IP e TCP (\texttt{ttl}, \texttt{len}, \texttt{proto}, \texttt{sport}, \texttt{dport});
\item informazioni Modbus decodificate (\texttt{function\_code}, \texttt{data\_length});
\item dimensione complessiva del pacchetto.
\end{itemize}

Le feature vengono trasformate tramite lo \texttt{StandardScaler} e passate al modello per ottenere il punteggio di decisione (\texttt{decision\_function}).
Se il punteggio risulta inferiore alla soglia salvata (\texttt{threshold}), il pacchetto viene marcato come anomalo.

\subsubsection{Threshold e Valutazione}
La soglia di rilevamento è definita dinamicamente in fase di training come percentile dei punteggi di decisione osservati sui dati d’attacco, solitamente il 65° percentile.
Questo consente un bilanciamento ottimale tra sensibilità e robustezza ai falsi positivi.
Durante la validazione sul dataset “clean”, viene calcolata la percentuale di campioni erroneamente classificati come anomalie, utile per calibrare il valore di soglia finale.

\subsubsection{Integrazione con il Sistema di Monitoraggio}
Nel flusso operativo in tempo reale:
\begin{enumerate}
\item Il pacchetto viene decodificato dal \textit{Protocol Handler};
\item Le feature vengono estratte e scalate;
\item Il modello ML produce un punteggio e una classificazione binaria (\textit{normal/anomaly});
\item Se viene rilevata un’anomalia, l’evento è notificato al modulo di \textit{alert management}, in modo analogo al servizio Z-Score.
\end{enumerate}

\subsubsection{Vantaggi e Considerazioni}
L’approccio \texttt{Isolation Forest} consente di rilevare comportamenti anomali anche in assenza di regole esplicite o soglie statiche, risultando particolarmente efficace per:
\begin{itemize}
\item pattern complessi o non lineari nei flussi SCADA;
\item deviazioni graduali dovute a compromissioni lente;
\item reti multi-protocollo con dinamiche diverse tra dispositivi.
\end{itemize}

Rispetto al modulo \texttt{ZScoreService}, la componente ML fornisce un’analisi più profonda del contesto, ma richiede una fase di addestramento supervisionato e un’accurata selezione delle feature per ciascun protocollo industriale.
...
\begin{figure}[!ht]
    ...
    \node[box, fill=orange!20, below=of discovery] (db) {\includegraphics[width=2.5cm]{img/side-sniff-image/neo4_logo.png}\\Neo4j Database};
...
\begin{figure}[!h]
    ...
    \node[box, fill=orange!20, below=of discovery, yshift=-2.5cm] (db) {\includegraphics[width=2.5cm]{img/side-sniff-image/neo4_logo.png}\\Neo4j Database};
